...
class SincConv1d(nn.Module):
    ...
    def _sinc_onnx(self, x):
        """
        手动实现 sinc 函数以兼容 ONNX 导出
        sinc(x) = sin(pi * x) / (pi * x)
        """
        pi = 3.141592653589793
        pi_x = pi * x
        # 针对 x 为 0 的特殊情况处理（sinc(0)=1），但在卷积核生成中，
        # 通常 x 的点位会是类似 [-1, -0.5, 0, 0.5, 1]，0 的位置需要通过 where 绕过
        # 此处使用 torch.where 保证导出的 ONNX 图逻辑闭环
        return torch.where(x == 0, torch.ones_like(x), torch.sin(pi_x) / pi_x)
    ...
    def forward(self, x):  # x: [B,1,T]
        ...
        # 将原来的 torch.sinc 替换为我们手写的 _sinc_onnx
        h = 2 * f2 * self._sinc_onnx(2 * f2 * n) - 2 * f1 * self._sinc_onnx(2 * f1 * n)
