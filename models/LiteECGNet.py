...
class LiteECGNet(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.num_flat_features = self._get_flatten_size(config['segment_len'], config['fs'])
    ...
    def _get_fc_input_size(self, config):
        with torch.no_grad():
            # 创建一个临时的模拟输入
            dummy_input = torch.zeros(1, 1, config['segment_len'])
            # 运行 forward 的前半部分（卷积部分）
            x = self.sinc(dummy_input)
            x = F.avg_pool1d(x, 2, 2)
            s2 = F.silu(self.bn7(self.conv7(dummy_input)))
            # 模拟补齐逻辑
            t1, t2 = x.size(2), s2.size(2)
            if t1 < t2: x = F.pad(x, (0, t2 - t1))
            elif t2 < t1: s2 = F.pad(s2, (0, t1 - t2))
            x = F.silu(self.bn32(self.pw32(torch.cat([x, s2], dim=1))))
            x = self.stage3(self.stage2(self.stage1(x)))
            # 获得展平后的确切长度
            self.num_flat_features = x.numel() 
            print(f"Detected FC input size: {self.num_flat_features}")
    def _get_flatten_size(self, segment_len, fs):
        """通过模拟一次前向传播来获取展平后的维度"""
        with torch.no_grad():
            # 1. 创建一个跟输入数据形状一样的 dummy tensor
            dummy_input = torch.zeros(1, 1, segment_len)
            
            # 2. 模拟 forward 里的所有卷积/池化操作
            s1 = self.sinc(dummy_input)
            s1 = F.avg_pool1d(s1, kernel_size=2, stride=2)
            s2 = F.silu(self.bn7(self.conv7(dummy_input)))
            
            # 对齐逻辑 (必须和 forward 里的逻辑一模一样)
            min_t = min(s1.size(2), s2.size(2))
            s = torch.cat([s1[:, :, :min_t], s2[:, :, :min_t]], dim=1)
    ...
    def forward(self, x):  # x: [B,1,T]
        ...
        t2 = s2.size(2)
        if t1 < t2:
        # pad format: (pad_left, pad_right) for 1D padding on last dim
         s1 = F.pad(s1, (0, t2 - t1), mode='constant', value=0)
        elif t2 < t1:
            s2 = F.pad(s2, (0, t1 - t2), mode='constant', value=0)
