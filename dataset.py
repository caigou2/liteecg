...
# -*- coding: utf-8 -*-
"""
ECG Lightweight Anomaly Detector - Chazal Dataset Split (8:2 Train/Val Split)
------------------------------------------
...
class ECGSegments(Dataset):
    def __init__(self,
        ...
        # ---------------------------------------------------------
        # 1. 严格按照 Chazal (2004) 论文划分的记录号 [cite: 8] [cite: 62]
        # ---------------------------------------------------------
        
        # Dataset 1 (DS1): 22 个记录
        ds1_records = ['101', '106', '108', '109', '112', '114', '115', '116', 
                       '118', '119', '122', '124', '201', '203', '205', '207', 
                       '208', '209', '215', '220', '223', '230']
        
        # Dataset 2 (DS2): 22 个记录，严格用于最终测试
        ds2_records = ['100', '103', '105', '111', '113', '117', '121', '123', 
                       '200', '202', '210', '212', '213', '214', '219', '221', 
                       '222', '228', '231', '232', '233', '234']

        # 2. 使用随机种子对 DS1 进行确定性打乱，保证每次运行划分结果一致
        rng = np.random.RandomState(self.random_state)
        ds1_shuffled = ds1_records.copy()
        rng.shuffle(ds1_shuffled)
        
        # 计算 8:2 的分割点 (22 * 0.8 ≈ 17.6 -> 取 17)
        split_idx = int(len(ds1_shuffled) * 0.8)

        # 3. 直接根据 split 参数分配需要读取的文件列表
        if self.split == 'train':
            records = ds1_shuffled[:split_idx]      # 训练集: DS1 的前 17 个记录
        elif self.split == 'val':
            records = ds1_shuffled[split_idx:]      # 验证集: DS1 的后 5 个记录
        elif self.split == 'test':
            records = ds2_records                   # 测试集: 完整的 DS2 (22个记录)
        else:
        ...
        for rid in pbar:
            ...
            # 增加直接的防呆检查，如果本地确实漏了某个文件，第一时间报错抛出
            if not os.path.exists(record_path + '.dat'):
                raise FileNotFoundError(f"在 {self.root} 目录下找不到文件: {rid}.dat！请检查路径或数据完整性。")
